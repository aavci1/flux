...
struct LineChart {
    ...
    Property<Color> backgroundColor = Color::hex(0xecf0f1);
    Property<Size> size = Size{400, 250};

    LayoutInfo layout(const Rect& bounds) const {
        return LayoutInfo(bounds, static_cast<Size>(size));
    }
    ...
    void render(RenderContext& ctx, const LayoutInfo& layout) const {
        ...
        Rect chartArea = {
            layout.bounds.x + padding.left,
            layout.bounds.y + padding.top,
            layout.bounds.width - padding.horizontal(),
            layout.bounds.height - padding.vertical()
        };

        // Draw line
        Color lineCol = resolveColor(lineColor);
        ...
        ctx.drawText(static_cast<std::string>(title),
                    {layout.bounds.x + 10, layout.bounds.y + 10},
                    16, Color::hex(0x2c3e50), FontWeight::bold, TextAlign::left);
    }
...
struct BarChart {
    ...
    void render(RenderContext& ctx, const LayoutInfo& layout) const {
        ...
        // Draw bars
        Color barCol = resolveColor(barColor);
...
struct DoughnutChart {
    ...
    void render(RenderContext& ctx, const LayoutInfo& layout) const {
        ...
        // Center and radius
        float centerX = layout.bounds.x + layout.bounds.width / 2;
        float centerY = layout.bounds.y + layout.bounds.height / 2;
        float radius = std::min(layout.bounds.width, layout.bounds.height) / 2 - 40;
        ...
        for (size_t i = 0; i < dataVec.size(); i++) {
            ...
            Color segmentColor = i < colorsVec.size() ? resolveColor(colorsVec[i]) : Color::hex(0x95a5a6);
